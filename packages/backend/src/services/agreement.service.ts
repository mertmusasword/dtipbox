...
export async function ensureAgreementTablesExist(): Promise<void> {
  // Prisma schema (db push / migrate) directly manages agreements, agreement_versions, and agreement_acceptances tables.
  // We perform a safe existence check to verify table availability without multi-statement prepared statement errors.
  try {
    await prisma.agreement.findFirst({ select: { id: true } });
  } catch {
    // Tables will be populated via Prisma db push/migrations
  }
}
